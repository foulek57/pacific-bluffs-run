Config = {
    -- Filling configuration
    fillDuration = 0.1*60*100000, -- 7 minutes en millisecondes
    drainDuration = 7*60*100000,
    -- Max speed in miles per hour (converted to meters per second)
    maxSpeedMph = 60.0,
    -- Allowed vehicle models
    allowedVehicles = {
        "hauler"
    },
    -- Remplissage
    refillLocations = {
        { x = 1240.08, y = -1479.96, z = 34.76 }, -- A cot� de la caserne de pompiers
    },
    -- Livraison
    deliveryLocations = {
        { x = 959.43, y = -1416.38, z = 31.30 } -- A d�finir une fois le mapping mis
    },

    GainsJob = { min = 50, max = 150 }, -- Gains de la société
    PlayerGains = { min = 450, max = 650 } -- Gains du joueur
}
